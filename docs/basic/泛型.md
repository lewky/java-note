...
lastmod: 2021-05-01T22:34:12+08:00
-->
## 泛型（Generic）
...
总结：
* 如果要从集合中读取类型`T`的数据，并且不能写入，可以使用`? extends T`通配符；(Producer Extends)
* 如果要从集合中写入类型`T`的数据，并且不需要读取，可以使用`? super T`通配符；(Consumer Super)
